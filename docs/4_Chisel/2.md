# Scala快速入门
为大家整理了一些关于Scala的入门教程以及手册，下边是一些粗略的对于Scala的语法学习，方便快速过渡到Chisel的学习当中。如果要深入了解Scala，请自行移步附录中进一步学习。

[Scala 菜鸟编程](https://www.runoob.com/scala/scala-basic-syntax.html)
...
## 5. 字符串和集合
`s.equals(s1)` `s.equalsIgnoreCase(s1)` `s.indexOf`
...
// 给List后边整体拼了一个aaa，而不是在list里边拼一个aaa
...
```
下午把scala的语法补充完整，然后开始chisel语法的学习
